...
(function($) {
    ...
    // brownish scale
    var color_scale10 = ['#E2E7F7', '#CDD5E8', '#B7C2D8', '#A2B0C9', '#8D9DB9', '#778BAA', '#62789A', '#4D668B', '#37537B', '#22416C'];
    ...
    function MapColorizer(elem, options) {
        ...
        var defaults = {
            url_geoson: "data/kuntarajat-2018.json", // url to fetch geojson polygons
            style_func: styleFunc, // styles for areas
            style_hover: {weight: 3, color: "#fff"}, // hover style
            min_zoom: 4, // min zoom level
            max_zoom: 6, // max zoom level
            show_full_screen_toggle : false, // show fullscreen toggler
            on_click: onClick, // event handler
            on_mouse_over: onMouseOver, // event handler
            on_mouse_out: onMouseOut, // event handler
            scale_mode: 1, // color scale count: 1 or 2
            color_scale: color_scale10, // array of colors
            overlay_opacity: 0.5, // background image/tiles opacity
            ...
            self.arealayer = L.geoJson(self.geodata.features, {
                ...
                filter: function (feature, layer) {
                    ...
                    for (var i = 0; i < OPT.area_list.length; i++) {
                        if (feature.properties.NATCODE == OPT.area_list[i])
                            return true;
                    }
                    return false;
                },
...
//            var options = {padding: [-10,-10]};
        ...
        function hilightArea(e) {
            ...
            var value = parseFloat(self.datamap[prop.NATCODE]);
            if (!value)
                value = 0;

            value = self.datamin ? "&nbsp;"+value : "";
            var ico = L.divIcon({className: 'areanamehover',
                                "html": "<span>"+prop.NAMEFIN + value + "</span>"});
 ...
 //           self.hover_mark = L.marker(layer.getBounds().getCenter(),
  //                              {icon: ico, clickable: false}).addTo(self.map);
        ...
        function get_area_color(properties) {
            ...
            var value = self.datamap[properties.NATCODE];
//            if (!value)
